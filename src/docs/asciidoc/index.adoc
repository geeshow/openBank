...
operation::customer-list[snippets='response-fields,curl-request,http-response,links']
...
[[resources-interest-check]]
=== 이자 금액 예상 조회

`GET` 요청을 사용해서 해당 계좌의 거래내역을 조회 할 수 있다.

operation::check-interest[snippets='curl-request,http-request,http-response,response-body,response-fields,response-headers,links']
...
operation::get-interest-list[snippets='curl-request,http-request,http-response,response-body,response-fields,response-headers,links']
